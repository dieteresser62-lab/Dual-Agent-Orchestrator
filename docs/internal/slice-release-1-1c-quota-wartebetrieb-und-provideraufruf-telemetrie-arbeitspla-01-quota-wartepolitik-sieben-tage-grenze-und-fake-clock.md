# Slice 01 – Quota-Wartepolitik, Sieben-Tage-Grenze und Fake Clock
...
Quota-Wartepolitik, Sieben-Tage-Grenze und Fake Clock
...
Der aktive Branch entspricht `feature/orchestrator-stabilization-1-1c`. Vor der
Umsetzung waren ausschließlich die vom Orchestrator angelegten, unversionierten
Audit- und Slice-Artefakte sichtbar. Die produktiven Änderungen bleiben auf die
acht im freigegebenen Slice genannten Pfade begrenzt; das Slice-Artefakt wird nur
außerhalb der verwalteten Auditblöcke ergänzt.
...
- Die Runtime-Defaults betragen nun 604800 Sekunden maximale Provider-
  Resetspanne und 3600 Sekunden Heartbeat-Intervall; CLI- und
  Umgebungsüberschreibungen behalten ihre bisherige Präzedenz.
- Die Auto-Wait-Grenze bewertet ausschließlich die UTC-normalisierte Spanne bis
  zum Provider-Reset. Die Sicherheitsmarge beeinflusst nur den persistierten
  Resume-Zeitpunkt.
- Die injizierbare Warteschleife meldet Eintritt, Reset-Grenzübertritt und
  Wiederaufnahme getrennt. Periodische Heartbeats enden am Reset, sodass die
  Sicherheitsmarge keine zusätzlichen Statusmeldungen erzeugt.
- Fake-Clock-, Grenzwert-, Offset-, Default-, Dokumentations- und
  Unterbrechungstests sichern den Vertrag ohne reales Warten ab.
...
Der erste Lauf identifizierte vier fehlerhafte Erwartungen des neuen
Grenztests zur bereits bestehenden UTC-Normalisierung und zur persistierten
manuellen Resetinformation; der Test wurde entsprechend präzisiert. Der
abschließende fokussierte Lauf ist unten vor Übergabe dokumentiert. Die
autoritative Gesamtmatrix führt ausschließlich der Orchestrator aus.

Abschlussergebnis: `180 passed in 69.57s`.
...
Arbeitsplan: `docs/internal/release-1-1c-quota-wartebetrieb-und-provideraufruf-telemetrie-implement-review-414a32cf.md`
