# Slice 01 – Enger Antigravity-Toolschema-Retry mit vollständigem Attemptabschluss
...
Enger Antigravity-Toolschema-Retry mit vollständigem Attemptabschluss
...
- Branch vor Umsetzung geprüft: `feature/orchestrator-stabilization-1-1d`.
- Ausgangsstatus enthielt ausschließlich die beiden vom Orchestrator erzeugten,
  unversionierten Audit-/Slice-Dokumente.
- Produktive Änderung bleibt auf den sechs freigegebenen Pfaden; bestehende
  allgemeine NETWORK-Retrygrenzen, Reviewparser und Recordtypen bleiben
  unverändert.
...
- Exakte Antigravity-`LineNumber`-Schemasignatur aus sanitisiertem
  `provider_data` als eigene Fehlerklasse klassifiziert; Prosa- und
  Struktur-Negativfälle bleiben hart.
- Automatische Fortsetzung der neuen Klasse auf genau einen bereits
  fingerprintgebundenen Resume begrenzt; vorangegangene Fehler derselben
  Operation verbrauchen diese Grenze, allgemeine NETWORK-Limits bleiben
  erhalten.
- Providerattempt-Bridge um terminalen Vorgängerzwang, fail-closed
  Zwei-Attempt-Grenze und datensparsame Start-/Terminal-Logs ergänzt.
- Fehlattempts persistieren injiziert gemessene Dauer und ausschließlich
  allowlist-normalisierte vorhandene Usage; Modell und JSON-Schema wurden
  synchron erweitert.
- Synthetische Regressionen decken Adapterhülle, Klassifikation, State-Roundtrip,
  Attemptabschluss/-cap, Projektion, Workflowreihenfolge sowie historische
  Ketten ohne neue Fehlerklasse ab.

## Ausgeführte Validierung mit Ergebnis

- Fokussierter Lauf gemäß Arbeitsplan: 371 Tests bestanden in 82,94 Sekunden.
- `git diff --check`: bestanden; lediglich die bestehende Git-Hinweismeldung zur
  künftigen CRLF-Normalisierung wurde ausgegeben.
- Die autoritative vollständige Validierungsmatrix bleibt dem Orchestrator
  vorbehalten.
...
- Bewusst enge Erkennung: Ändert Antigravity Statusfeld oder `error.message`-
  Struktur, fällt der Fehler fail-closed auf die bestehende harte
  Klassifikation zurück.
- Die Bridge ist die zweite physische Grenze bei Resume nach einem Crash zwischen
  Attemptterminal und Workflow-State-Checkpoint.
