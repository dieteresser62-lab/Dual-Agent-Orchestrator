...
**Revision 10** — Claude verwendet verbindlich Sonnet mit Effort `high`. Sein erstes Slice-Review erhält ausschließlich die seit dem persistierten Slice-Start geänderten Pfade und Hunks sowie destillierte Akzeptanzkriterien, Finding-Records und die fingerprintgebundene Attestierung; offene Repositoryerkundung und unveränderte Dateien bleiben ausgeschlossen. Nach jeder inhaltlichen Codex-Korrektur erhält Claude nur den Korrekturdelta seit seinem zuletzt geprüften Fingerprint plus aktualisierte Records und Attestierung. Eine ausschließlich formal abgelehnte Antwort wird aus der abgelehnten Antwort, dem Validatorfehler und dem Ausgabecontract repariert, ohne Implementierungsevidenz erneut zu übertragen oder ein neues fachliches Review zu behaupten. Antigravity erhält weiterhin einmalig den vollständigen finalen Slice-Diff.

**Revision 9** — Deterministische Validierung wird je kanonischem Diff-Fingerprint genau einmal durch den Python-Orchestrator ausgeführt und als strukturierte Attestierung an die Reviewkette gebunden. Codex darf während der Entwicklung beliebig fokussiert testen, aber weder sein Bericht noch eine Reviewerbehauptung ist die maßgebliche Abnahmevalidierung. Claude und Antigravity prüfen dieselbe Attestierung und führen die Vollsuite nicht erneut aus. Nach jeder inhaltlichen Korrektur entsteht ein neuer Fingerprint und damit zwingend eine neue Orchestrator-Attestierung; gezielte Zusatzvalidierung kann ein Reviewer als Finding-Akzeptanztest verlangen. Claudes vollständiges Reviewpaket wird verlustfrei in manifestierte, begrenzte Chunks zerlegt; das Lesebudget entspricht exakt Manifest plus allen Chunks, damit eine starre Werkzeuggrenze nicht die Implementierungsprüfung verhindert. Der Adapter-/Versions-Smoke einschließlich negativem Schreibtest bleibt eine separate, nur bei Installation oder Versionswechsel auszuführende Fähigkeitsevidenz.
...
**Revision 7** — Claude-Reviews erhalten ein einziges vorgebautes, begrenztes Reviewpaket statt offener Repositoryerkundung. Damals blieben `Read` und der exakt freigegebene Review-Harness verfügbar; Revision 9 ersetzt den Harness im Normalreview durch die Orchestrator-Attestierung. Der Prompt begrenzt den Lauf auf höchstens sechs Werkzeugaufrufe und die strukturierte Antwort auf 12.000 Zeichen. Antigravitys Abschlussreview wird präzisiert: Prüfgegenstand ist immer der vollständige Diff des aktuellen Slice seit dessen persistiertem Start-Commit einschließlich aller Claude-/Codex-Korrekturrunden. Ein letzter Korrekturdelta darf hervorgehoben werden, ersetzt den Gesamtdiff aber nie.
...
2. **Codex** implementiert Slice *n* einschließlich Tests und darf während der Entwicklung fokussierte oder vollständige Testläufe ausführen. Nach `IMPLEMENTATION_READY` ermittelt der Orchestrator den kanonischen Diff-Fingerprint, führt die gemäß §6.2 erforderliche Validierungsmatrix selbst genau einmal aus und persistiert eine strukturierte Attestierung mit Attestierungs-ID, Fingerprint, Befehlen, Exitcodes, Vollständigkeit, kompaktem Ergebnis und Ausgabedigest. Nur diese Attestierung ist für die Freigabe maßgeblich und wird unter `## Ergebnisse` in die Slice-MD projiziert.
3. **Claude Code** reviewt. Claude erhält dafür ein einziges, vom Orchestrator vorgebautes Reviewpaket mit Anforderungen, Planentscheidungen, Findings, Dateischnappschüssen, kanonischem Diff und passender Validierungsattestierung. Offene Repositoryerkundung und ein erneuter Volltest entfallen; im Normalreview ist nur `Read` verfügbar. Die eingesparte Laufzeit und Quota dienen der adversarialen Prüfung von Implementierung, Invarianten, Fehlerpfaden, Sicherheitsgrenzen, Resume/Idempotenz und Testlücken. Bei Mängeln: zurück zu Schritt 2. Eine zusätzliche gezielte Validierung formuliert Claude als Finding-Akzeptanztest; der Orchestrator führt sie auf dem korrigierten Fingerprint aus.
4. **Codex** korrigiert und antwortet unter `## Review-Antworten von Codex`. Jede inhaltliche Änderung invalidiert Fingerprint und Attestierung; der Orchestrator validiert den neuen Stand erneut, danach zurück zu Schritt 3. Die Schritte 2 bis 4 wiederholen sich, bis Claude freigibt.
5. **Antigravity** reviewt den von Claude freigegebenen Slice ohne erneuten Volltest. Sein Reviewpaket enthält immer den vollständigen Slice-Diff vom persistierten Start-Commit des Slice bis zum Claude-freigegebenen Endstand, dieselbe für diesen Fingerprint gültige Attestierung und sämtliche früheren Korrekturrunden. Der letzte Korrekturdelta darf den vollständigen Slice-Diff nie ersetzen. Bei Mängeln: zurück zu Schritt 4.
6. Nach Freigabe durch beide Reviewer autorisiert Antigravity den Commit. Der Orchestrator führt die Commit-Sicherheitsprüfung durch und staged ausschließlich erlaubte Pfade. Unerwartete Dateien, ein seit der Attestierung veränderter Diff oder eine nicht zum Freigabefingerprint passende Attestierung blockieren den Commit. Danach erzeugt der Orchestrator den lokalen Commit für Slice *n* mechanisch.
...
Vor diesen Reviews führt der Orchestrator die vollständige branchweite Validierungsmatrix genau einmal aus und bindet die Attestierung an den Branch-Diff-Fingerprint. Alle drei Instanzen prüfen denselben Reviewgegenstand und dieselbe Attestierung; sie starten die Vollsuite nicht jeweils erneut. Nach einer Korrektur werden Fingerprint und Attestierung neu erzeugt, bevor das Endreview von vorn beginnt.
...
- **Eine rote, unvollständige, fehlende oder fingerprintfremde Orchestrator-Attestierung blockiert jede Freigabe.** Einzige Ausnahme: Eine vollständig ausgeführte, erwartungsgemäß rote Matrix eines bewussten Contract-Slice darf nach der Red-State-Regel (§5) mit namentlich benannter Folge-Slice freigegeben werden. `INCOMPLETE`, fehlende oder fingerprintfremde Attestierungen fallen nie unter diese Ausnahme.
...
| Orchestratorische Validierungsattestierung im State/Auditpaket, nicht in Agentenantworten | `VALIDATION_ATTESTATION: <id> \| <diff-fingerprint> \| PASS\|FAIL\|INCOMPLETE` |
...
- `*_APPROVAL: YES` nur bei einer vollständigen, grünen und zum Reviewgegenstand passenden `VALIDATION_ATTESTATION` (Ausnahme: vollständige rote Matrix nach der Red-State-Regel, §2.5). Ein von Codex oder einem Reviewer ausgegebenes `VALIDATION_RESULT` ist im State-v3-Contract unzulässig und kann keine Attestierung ersetzen.
...
*Abnahme:* Alle drei Adapter starten und liefern auf einen Minimalprompt eine parsbare Antwort mit korrektem Abschlussmarker. Kein Modellname und kein plattformspezifischer Antigravity-Binaryname ist im Quelltext fest verdrahtet. Der Preflight dokumentiert je Rolle aufgelösten Befehl, Version und erkannte Fähigkeiten. Claudes Langprompt liegt als einziges externes, manifestiertes Reviewpaket vor; der normale Reviewprozess erhält nur dessen kurzen Verweis und `Read`, exakt einen Leseaufruf für Manifest und jeden begrenzten Chunk sowie ein kleines Antwortschema. Die Konkatenation aller Chunks muss bytegleich zum ursprünglichen Paket sein. Der Normaladapter spricht über die vom jeweiligen Workflow gelieferte Validierungsevidenz, ohne eine v3-Attestierung im aktiven v2-Pfad vorauszusetzen. Der Harness gehört ausschließlich zur getrennten Fähigkeitsevidenz und ist über explizite Diagnosebefehle beider Revieweradapter erreichbar, niemals über ihren Normalreview. Reviewerprozesse setzen weiterhin `PYTHONDONTWRITEBYTECODE=1`, ohne Testkommando-, Probe- oder Harnessvariablen in den Normalreview zu übernehmen. Für jede neue oder ungetestete Version greift ein Nutzergate, bis Minimalprompt, strukturierte Text-/Streamausgabe, Langprompt über den vorgesehenen Eingabekanal, Timeout, Auth-/Quota-/Netzfehler und Prozessende verifiziert sind. Live-Smokes laufen nach Installation oder Versionsänderung, nicht bei jedem Start, weil sie Tokens beziehungsweise Quota verbrauchen.
...
Claude arbeitet in allen Runden mit Sonnet und Effort `high`. Runde 1 ist ein gezieltes Änderungsreview über den Slice-Diff seit dem persistierten Start-Commit, kein Repositoryreview. Folgerunden erhalten nur den Korrekturdelta seit Claudes zuletzt geprüftem Fingerprint, die fortgeschriebenen Finding-Records und die neue Attestierung. Eine reine Contract-/Markerreparatur ist kein neuer Reviewdurchlauf und darf die Implementierungsevidenz nicht erneut übertragen.
...
Die Matrix wird pro kanonischem Diff-Fingerprint einmal vom Orchestrator ausgeführt. Ihre strukturierte Attestierung ist Eingabe der Claude- und Antigravity-Contracts; Modelle führen die Matrix nicht erneut aus und dürfen kein eigenes `VALIDATION_RESULT` als Ersatz melden. Jede inhaltliche Änderung invalidiert die Attestierung. Reviewer können zusätzliche gezielte Befehle als Finding-Akzeptanztest verlangen; auch deren Ausführung und Ergebnis erfasst der Orchestrator in der nächsten Attestierung.

*Abnahme:* Eine Konfiguration „`npm test` immer, `npm run build:engine` zusätzlich bei Änderungen unter `engine/`" ist abbildbar, und die Zusatzvalidierung läuft nachweislich nur bei betroffenem Diff. Zwei Reviewer desselben unveränderten Fingerprints verursachen keinen zweiten Testlauf; nach einer Codekorrektur entsteht genau eine neue Attestierung.
...
Codex kann während der Entwicklung Tests ausführen; diese Läufe sind Diagnose, nicht die Freigabequelle. Nach jedem `IMPLEMENTATION_READY` beziehungsweise jeder Korrektur führt der Python-Orchestrator die für den kanonischen Diff erforderliche Matrix genau einmal aus. Die unveränderliche Attestierung enthält mindestens Attestierungs-ID, Diff-Fingerprint, vollständige/fehlende Befehle, Exitcodes, PASS/FAIL/INCOMPLETE, kompaktes Ergebnis und SHA-256-Ausgabedigest. Sie wird vor jedem Review gegen den aktuellen Fingerprint geprüft und als strukturierter Contract-Input an Claude und Antigravity übergeben.

Reviewer führen die Vollsuite nicht erneut aus und geben kein eigenes `VALIDATION_RESULT` aus. Eine Freigabe ohne passende vollständige grüne Attestierung scheitert fail-closed. Ändert sich der Reviewgegenstand inhaltlich, verfällt die Attestierung; reine deterministische Projektion bereits validierter Review-/Auditrecords in verwaltete Dokumentabschnitte gehört nicht zum fachlichen Reviewgegenstand und erzeugt keinen Selbstinvalidierungszyklus. Geforderte gezielte Zusatztests laufen ebenfalls mechanisch im Orchestrator und werden Bestandteil der nächsten Attestierung.

Der negative Repository-Schreibtest und die CLI-Runtime-/Netzwerk-Smokes prüfen die Adapterfähigkeit, nicht jeden Slice. Sie laufen nach Installation, Versionsänderung oder expliziter Diagnose und werden nicht pro Claude-/Antigravity-Review wiederholt.
