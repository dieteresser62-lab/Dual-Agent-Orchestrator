...
- Runs have no family, remediation-round, or record-level handoff binding. A plan may be approved only after every plan Finding is closed. After all implementation Slices are committed, the same `IMPLEMENT` run performs its final full-branch review. Before the configured acceptance-review limit, new final-review Findings are rendered into an ordinary work document in the Inbox; watch mode starts that document exactly like a human-authored task, with no required identifier, fingerprint, or record correlation to the preceding run. At the limit, the task moves to the failed output, no new document is created, the last findings remain auditable, and committed Slices stay on the target branch.
...
- A Finding belongs immutably to the Slice in which it was opened. Findings never cross Slice boundaries: Claude closes an implemented Finding or an accepted rejection, and every dispositioned ordinary Finding that Claude does not close becomes a `BLOCKER`.
- Codex cannot close or change the class of reviewer-owned Findings. It may reject an ordinary `FINDING`, but it cannot reject a `BLOCKER`; the stop reasons `CONTRACT-UNCLEAR`, `OPERATOR-PREREQUISITE-MISSING`, and `SCOPE-EXTENSION-REQUESTED` remain available.
- Slice approval and Slice commit are different gates. The sole commit condition is that the record-derived Slice cohort contains no open blocker.
...
- Only the reporting reviewer may close its Finding. Codex must provide exactly one reasoned `finding_dispositions` response for every open Finding during plan, implementation, and same-Slice correction work. Claude expresses escalation by leaving an ordinary Finding open and denying the review, not through an output field; the canonical reviewer reduction then emits the record-bound `escalated` transition to `BLOCKER`.
- The final full-branch review only discovers new Findings or records occurrences of known signatures; it has no correction work unit. Its result completes the implementation run, and any new work is expressed solely through the ordinary Inbox follow-up document.
...
- Plan and Slice loops count complete implementer/reviewer pairs and stop negatively without a commit at the configured per-loop round limit. Every Slice review, including convergence rounds, is measured against the immutable Slice start. The separately configured outer limit counts acceptance reviews and permits no further run once reached.
