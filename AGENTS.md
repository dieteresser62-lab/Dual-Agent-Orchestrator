...
- A typed `BLOCKER` acceptance command is measured by the orchestrator at the pre-change and post-change fingerprints, and `fixed` requires recorded `FAIL` then `PASS`; a pre-green result cannot prove `fixed`. `partial` records evidence and remaining work but stays open. Prose acceptance remains opaque and has no inferred substitute check.
