...
- Every Slice-owned Finding receives exactly one recorded decision before Slice commit: fixed; rejected with named evidence as `not_a_defect`, `outside_scope`, or `already_fixed`; or routed by Claude to a named later approved-plan Slice or to branch planning. Codex may propose a disposition or responsibility target, but cannot close, reject, route, or reclassify a reviewer-owned Finding.
- Claude may reclassify `BLOCKER` to `OBSERVATION` only from fingerprint-bound evidence and records the transition. Routing or reclassification alone never disposes an open `BLOCKER`; it must first be fixed or evidentially rejected.
- Slice approval and Slice commit are different gates. Immediately before commit, the record-derived Slice cohort must contain no open blocker; every cohort Finding must be closed or validly routed; and no cohort Finding may still be owned by the current Slice. A routed later Slice must still be pending and bind both adequate approved scope and the Finding's request-bound acceptance criterion.
- The first Slice review is discovery. Every denied later review is a convergence round and progresses only by closing or validly forwarding a previously local Finding, or by a new fingerprint with attested remedial effect. A stalled round ends the work unit negatively and gate-free without a Slice commit.
