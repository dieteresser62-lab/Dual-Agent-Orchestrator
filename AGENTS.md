...
- Runs have no family, cycle, remediation-round, or record-level handoff binding. A plan may be approved only after every plan Finding is closed. After all implementation Slices are committed, the same `IMPLEMENT` run performs its final full-branch review. New final-review Findings are rendered into an ordinary work document in the Inbox; watch mode starts that document exactly like a human-authored task, with no required identifier, fingerprint, or record correlation to the preceding run.
