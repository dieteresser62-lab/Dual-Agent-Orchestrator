...
def test_pre_target_class_round_exit_reducer_is_named_and_rejected_fail_closed() -> None:
    assert artifact_models.PRE_TARGET_CLASS_ROUND_EXIT_REDUCER_VERSION == (
...
def test_pre_scope_extension_reducer_is_named_and_rejected_fail_closed() -> None:
    assert artifact_models.PRE_SCOPE_EXTENSION_REDUCER_VERSION == (
...
def test_pre_target_finding_lifecycle_reducer_is_named_and_rejected_fail_closed() -> None:
    assert artifact_models.PRE_TARGET_FINDING_LIFECYCLE_REDUCER_VERSION == (
...
def test_scope_extension_record_roundtrips_paths_categories_and_reason() -> None:
    payload = ScopeExtensionPayload(
        work_unit_id="2",
        slice_id="1",
        source_request_id=CODEX_REQUEST_ID,
        stop_rule_id="SCOPE-EXTENSION-REQUESTED",
        rationale="Required paths: docs/guide.md, src/runtime.py\nWhy required: finish",
        additions=(
            ScopeExtensionPathPayload("docs/guide.md", "documentation"),
            ScopeExtensionPathPayload("src/runtime.py", "productive"),
        ),
    )
    ...
    with pytest.raises(ArtifactValidationError, match="category is invalid"):
        replace(
            payload,
            additions=(ScopeExtensionPathPayload("src/runtime.py", "unknown"),),
        )
...
def test_finding_escalation_is_a_closed_reviewer_owned_record_move() -> None:
    escalation = FindingTransitionPayload(
        ...
        "open",
        "The rejected Finding is now a Blocker.",
    )
