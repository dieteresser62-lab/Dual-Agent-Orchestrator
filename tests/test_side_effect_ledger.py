...
def test_pending_audit_commit_is_resumable_during_final_review(
    tmp_path: Path,
) -> None:
    subprocess.run(
    ...
    audit_path = "docs/internal/audit.md"
    audit = tmp_path / audit_path
    audit.parent.mkdir(parents=True)
    audit.write_text("before\n", encoding="utf-8")
    (tmp_path / ".gitignore").write_text(".orchestrator/\n", encoding="utf-8")
    subprocess.run(
        ["git", "add", audit_path, ".gitignore"],
        cwd=tmp_path,
        check=True,
    )
    subprocess.run(
    ...
    audit.write_text("after\n", encoding="utf-8")
    changes = collect_repository_changes(
        tmp_path, prior, semantic_markdown_paths=(audit_path,)
    )
    expected_tree = preview_commit_tree(
        tmp_path, changes, force_non_executable_paths=(audit_path,)
    )
    bridge = _bridge(tmp_path, "pending-audit-run")
    _initialize_ledger(bridge)
    operation = (
        "audit_commit",
        audit_path,
    ...
    driver._mark_completed_side_effect = lambda effect_key: None
    driver.assert_structured_decision_context = (
        lambda: driver._reconcile_pending_side_effects(state)
    )

    committed = driver.finalize_audit(state)

    assert committed is not None and committed != prior
    replay = replay_artifacts(bridge.store.load_chain(), state.run_id)
    effect = next(
        item for item in replay.side_effects if item.effect_class == "git_commit"
    )
    assert effect.result == committed
