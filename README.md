...
Prüfbefehle stehen als Argumentvektoren in `default_command`, `product_command` oder einem Regel-`command`. Braucht ein Projekt Shell-Semantik oder einen Unterordner, rufen Sie die Shell ausdrücklich als Programm auf: `default_command = ["sh", "-c", "cd app && flutter test"]`. Die früheren Shell-Schlüssel werden beim Start abgewiesen. Muster sind repositoryrelativ, verwenden `/` und dürfen nicht mit `..` ausbrechen.
